...
**AI (LangChain).** All AI calls go through LangChain for a unified provider interface. [server/services/langchain.ts](server/services/langchain.ts) creates `ChatGoogleGenerativeAI` (Gemini 2.5 Flash) and `ChatOpenAI` (GPT-4o-mini) models. `getChatModel()` returns a model with Gemini as primary and OpenAI as automatic fallback via LangChain's `.withFallbacks()`. `toLangChainMessages()` converts internal message format to LangChain's `HumanMessage`/`AIMessage`/`SystemMessage`.

**AI suggestions.** [app/api/ai-suggestion/route.ts](app/api/ai-suggestion/route.ts) provides two modes: improve (spelling/grammar fixes, temp=0.1) and expand (adds detail, temp=0.8). Uses `getChatModel(…).invoke()` for non-streaming responses. Output is cleaned by `extractOutput()` to strip intros/sign-offs/code fences. Usage is tracked per-user with monthly resets via `aiSuggestionsCount` / `aiSuggestionsResetAt` on the User model. Two layers of rate limiting: in-memory by IP+user ID, and plan-tier quota from the database.

**AI chat (SnackAI).** [app/api/ai/chat/route.ts](app/api/ai/chat/route.ts) is a streaming SSE endpoint for in-note chat. [server/api/ai-chat.ts](server/api/ai-chat.ts) builds a system prompt from the current note context and streams via `getChatModel(…).stream()`. The client hook [hooks/use-lang-chat.ts](hooks/use-lang-chat.ts) handles SSE parsing, abort, and message state. The `AiChatPanel` component ([components/dashboard/ai-chat-panel.tsx](components/dashboard/ai-chat-panel.tsx)) renders the chat UI with a lightweight markdown renderer ([components/dashboard/chat-markdown.tsx](components/dashboard/chat-markdown.tsx)). Usage is counted against the same per-plan AI quota as suggestions.
