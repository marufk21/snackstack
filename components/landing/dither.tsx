...
export default function Dither({
  ...
  // Default to light mode if theme is not available (SSR or not mounted)
  const isDark = mounted && (resolvedTheme === "dark" || theme === "dark");

  // Default colors - adapt based on theme
  // Light mode: brighter purple waves on light background
  // Dark mode: darker purple waves on black background
  const defaultWaveColor: [number, number, number] =
    waveColor || (isDark ? [0.4, 0.1, 0.6] : [0.65, 0.35, 0.85]); // Brighter, more vibrant purple for light mode

  const defaultBaseColor: [number, number, number] =
    baseColor || (isDark ? [0.0, 0.0, 0.0] : [0.97, 0.97, 0.99]); // Light background for light mode
